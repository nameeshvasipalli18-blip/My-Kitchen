...
export const TripManager = ({
  ...
  setTripStarted,
  tripStarted,
  tripDefaultsEditable,
  setTripDefaultsEditable,
  isSavingTrip,
  setIsSavingTrip,
  onTripSaved,
  onItemAdded,
  ...
  const submitTrip = async () => {
    try {
      ...
      setSettlementResults(error.response?.data?.detail || 'Error submitting trip.');
    }
    return null;
  };
  ...
  const getTrips = async () => {
    try {
      ...
      setSettlementResults(error.response?.data?.detail || 'Error fetching trips.');
    }
  };
  ...
  const handleSaveBill = async () => {
    ...
    setShoppingTrip(resetShoppingTrip());
        ...
        <div className="shopping-trip-container">
          <div className="shopping-defaults">
            <div className="shopping-default-fields">
              <label className="shopping-default-field">
                <span>Date</span>
                <input type="date" value={shoppingTrip.date} disabled={tripStarted && !tripDefaultsEditable} onChange={(e) => setShoppingTrip((prev) => ({ ...prev, date: e.target.value }))} />
              </label>
              <label className="shopping-default-field">
                <span>Store</span>
                <select value={shoppingTrip.store} disabled={tripStarted && !tripDefaultsEditable} onChange={(e) => setShoppingTrip((prev) => ({ ...prev, store: e.target.value }))}>
              ...
              {customSplitParticipants && (
                <label className="shopping-default-field">
                  ...
                  <select multiple className="item-split-between-select" value={shoppingTrip.participants} disabled={tripStarted && !tripDefaultsEditable} onChange={(e) => {
                    const selectedOptions = Array.from(e.target.selectedOptions, (option) => option.value);
                    setShoppingTrip((prev) => ({ ...prev, participants: selectedOptions }));
                  }}>
                    {initialInputs.map((person, idx) => <option key={idx} value={person}>{person}</option>)}
                  </select>
                </label>
              )}
              <label className="shopping-default-field">
                <span>Paid by</span>
                <select className="item-paid-by-select" value={shoppingTrip.defaultPayer} disabled={tripStarted && !tripDefaultsEditable} onChange={(e) => setShoppingTrip((prev) => ({ ...prev, defaultPayer: e.target.value }))}>
                  <option value="">Select Default Payer</option>
                  {shoppingTrip.participants.map((person, idx) => <option key={idx} value={person}>{person}</option>)}
                </select>
              </label>
            </div>
          ...
          {tripCount > 0 && tripStarted && (
            <div className="save-bill-action">
              <button className={`lock-button${isSavingTrip ? ' save-trip-exiting' : ''}`} disabled={isSavingTrip} onClick={handleSaveBill}>
                Save Bill
              </button>
            </div>
          )}
